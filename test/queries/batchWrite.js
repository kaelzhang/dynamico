...
describe('#batchWrite', () => {
  describe('batchPut/batchWrite', () => {
    ...
    it('should put multiple items to DB using batchPut', () => Table.batchPut(batchPut).then(data => {
      assert.deepEqual({}, data.UnprocessedItems)
    }))

    it('should put multiple items to DB using batchWrite', () => Table.batchWrite(batchPut).then(data => {
      assert.deepEqual({}, data.UnprocessedItems)
    }))
  ...
  describe('batchDelete', () => {
    ...
    it('should delete multiple items from DB', () => Table.batchDelete(batchDelete).then(data => {
      assert.deepEqual({}, data.UnprocessedItems)
    }))

    it('should not delete unexisting items', () => Table.batchDelete({'aws.table.for.testing': [{name: '&'}]}))
  })
})
